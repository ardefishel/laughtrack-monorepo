  ...
  useEffect(() => {
    ...
    const verify = async () => {
      ...
      try {
        ...
        if (!active) return
        setStatus(resolveVerificationStatus(response.ok, payload?.code))
      } catch {
        if (!active) return
        setStatus('invalid')
      }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-6 py-16 text-foreground">
      <div className="w-full max-w-xl rounded-2xl border border-border bg-surface p-8 shadow-xl">
        <div className={`mb-6 inline-flex rounded-full border px-3 py-1 text-xs font-medium uppercase tracking-[0.2em] ${toneClasses}`}>
          Email verification
        </div>
