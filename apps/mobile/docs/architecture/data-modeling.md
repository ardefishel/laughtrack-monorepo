...
The same structure is now implemented for Note, Premise, Bit, and Setlist entities.
...
- File: `src/database/schemas/noteSchema.ts`
...
- File: `src/database/schemas/premiseSchema.ts`
- Owns table/column constants (`PREMISE_COLUMNS`) and table schema
- Owns DB record shape (`PremiseRecord`) with DB-native fields (`tags_json`, `source_note_id`, `updated_at`)
...
- Handles schema upgrades (premises added at version `2`, `source_note_id` column added at version `3`)
...
- File: `src/database/schemas/bitSchema.ts`
...
- File: `src/database/migrations.ts`
- Handles schema upgrades (bits included in initial schema)
...
- File: `src/domain/setlist.ts`
- Owns `SetlistSchema`, `SetlistItemSchema`, `SetlistNoteSchema`, and related setlist types
- Uses app-friendly field names and types (`updatedAt: Date`)

### 2) Database schema metadata

- File: `src/database/schemas/setlistSchema.ts`
- Owns table/column constants (`SETLIST_COLUMNS`) and table schema
- Owns DB record shape with DB-native fields (`items_json`, `tags_json`)

### 3) Watermelon model behavior

- File: `src/database/models/setlist.ts`
- Owns runtime model behavior (`updateSetlist` writer)
- Uses shared column constants instead of repeating string literals

### 4) Storage schema registration

- File: `src/database/schema.ts`
- Registers setlist table schema (`SETLIST_TABLE_SCHEMA`)
- File: `src/database/migrations.ts`
- Handles schema upgrades (setlists added at version `2`)

### 5) Mapping between layers

- File: `src/database/mappers/setlistMapper.ts`
- Converts DB records/models <-> domain type
- Handles snake_case <-> camelCase and JSON serialization for items/tags
...
2. Add DB table constants + `*Record` in `src/database/schemas/<entity>Schema.ts`.
